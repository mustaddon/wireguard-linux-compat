...
function installQuestions() {
	...
	RANDOM_PORT=$(shuf -i4000-9999 -n1)
	until [[ ${SERVER_PORT} =~ ^[0-9]+$ ]] && [ "${SERVER_PORT}" -ge 1 ] && [ "${SERVER_PORT}" -le 65535 ]; do
		read -rp "Server WireGuard port [1-65535]: " -e -i 443 SERVER_PORT
	done
...
function newClient() {
	...
	BASE_IP="${SERVER_WG_IPV6::-2}"
	until [[ ${IPV6_EXISTS} == '0' ]]; do
		read -rp "Client WireGuard IPv6: ${BASE_IP}" -e -i "$(printf "%02x" $DOT_IP)" DOT_IP
